    ...
    (defn create-rag-state-machine []
      {:states {:init {:description "Initialize the RAG pipeline"
                ...
                :retrieve-chunks {:description "Retrieve document chunks based on search phrases"
                                  :next :format-chunks}
                :format-chunks {:description "Format retrieved chunks for display"
                                :next :rerank-chunks}
                :rerank-chunks {:description "Rerank chunks by relevance"
                                :next :transact-retrieval-prompt}
                :transact-retrieval-prompt {:description "Save retrieval prompt to database"
                ...
                :transact-assistant-msg {:description "Save assistant message to database"
                                         :next :transact-retrieved-sources}
                :transact-retrieved-sources {:description "Save retrieved sources to database"
...
;; State machine execution engine
 #?(:clj
    (defn register-handler [state-machine state-name handler-fn]
      (assoc-in state-machine [:handlers state-name] handler-fn)))
 ...
 #?(:clj
    (defn execute-state [state-machine]
      (let [current-state (:current-state state-machine)
            handler (get-in state-machine [:handlers current-state])
        ...
        (if (nil? handler)
          (do
            ...
            state-machine)
          (try
            (let [{:keys [next-state updated-context]} (handler context)]
              (transition-to state-machine next-state updated-context))
            (catch Exception e
              (println (str "Error in state " current-state ": " (.getMessage e)))
              (-> state-machine
...
;; Define handlers for each state in the RAG pipeline
 ...
 #?(:clj
    (defn query-relaxation-handler [{:keys [params !dh-conn completion-messages] :as context}]
      (println "Starting query relaxation...")
      (reset! !response-states ["Ser etter dokumenter"])

      (let [extract-search-queries (query-relaxation (:promptRagQueryRelax params) completion-messages)]
        (println "Done query relaxation.")

        {:next-state :lookup-search-phrases
 ...
 #?(:clj
    (defn lookup-search-phrases-handler [{:keys [params !dh-conn extract-search-queries filter-by] :as context}]
      (println "Starting to lookup search phrases...")

      (let [search-phrase-hits (lookup-search-phrases-similar
        ...
        (println "Done lookup search phrases, found count:" (count search-phrase-hits))

        (if (empty? search-phrase-hits)
          (do
            (reset! !response-states ["Ingen søkefraser funnet"])
            (throw (ex-info "No search phrase found" {})))

          {:next-state :retrieve-chunks
           :updated-context (assoc context :search-phrase-hits search-phrase-hits)}))))
    ...
    (defn retrieve-chunks-handler [{:keys [params !dh-conn search-phrase-hits] :as context}]
      ...
      (let [retrieved-chunks (retrieve-chunks-by-id
        ...
        (if (empty? retrieved-chunks)
          ...
          {:next-state :format-chunks
           :updated-context (assoc context :retrieved-chunks retrieved-chunks)}))))

 #?(:clj
    (defn format-chunks-handler [{:keys [params !dh-conn retrieved-chunks extract-search-queries] :as context}]
      (println "Starting to format retrieved chunks...")

      (let [loaded-chunks (->> retrieved-chunks
                               (map-indexed
                                (fn [idx doc]
        ...
        (println "Done formatting retrieved chunks.")

        {:next-state :rerank-chunks
         :updated-context (assoc context
                                 :loaded-chunks loaded-chunks
                                 :formatted-message formatted-message)})))

 #?(:clj
    (defn rerank-chunks-handler [{:keys [params !dh-conn retrieved-chunks] :as context}]
      (println "Starting to rerank chunks...")
      (reset! !response-states ["Sorterer rekkefølgen"])

      (let [{:keys [loaded-chunk-ids loaded-docs full-prompt]} (rerank-chunks retrieved-chunks params)]
        (println "Done reranking chunks.")

        (when (empty? loaded-docs)
          (println "***** Error: No documents were loaded during reranking. *****"))

        {:next-state :transact-retrieval-prompt
         :updated-context (assoc context
                                 :loaded-chunk-ids loaded-chunk-ids
                                 :loaded-docs loaded-docs
                                 :full-prompt full-prompt)})))
 ...
 #?(:clj
    (defn transact-retrieval-prompt-handler [{:keys [params !dh-conn convo-id retrieval-prompt-msg-id full-prompt] :as context}]
      (println "Starting to transact retrieval prompt...")

      (db/transact-retrieval-prompt !dh-conn convo-id retrieval-prompt-msg-id full-prompt)

      (println "Done transacting retrieval prompt.")

      {:next-state :generate-response
       :updated-context context}))

 #?(:clj
    (defn generate-response-handler [{:keys [params !dh-conn convo-id extract-search-queries
                                             formatted-message full-prompt loaded-chunk-ids loaded-docs] :as context}]
      (println "Starting to generate response...")
      (reset! !response-states ["Skriver svar"])

      (let [generation-result (rag-generate !dh-conn convo-id extract-search-queries
                                            formatted-message full-prompt params loaded-chunk-ids loaded-docs)]

        (println "Done generating response.")

        {:next-state :transact-assistant-msg
         :updated-context (assoc context :generation-result generation-result)})))

 #?(:clj
    (defn transact-assistant-msg-handler [{:keys [params !dh-conn convo-id generation-result] :as context}]
      (println "Starting to transact assistant msg...")

      (db/transact-assistant-msg !dh-conn convo-id (:english_answer generation-result))

      (println "Done transacting assistant msg.")

      {:next-state :transact-retrieved-sources
       :updated-context context}))

 #?(:clj
    (defn transact-retrieved-sources-handler [{:keys [params !dh-conn convo-id retrieved-sources-msg-id formatted-message] :as context}]
      (println "Starting to transact retrieved sources...")

      (db/transact-retrieved-sources !dh-conn convo-id retrieved-sources-msg-id formatted-message)

      (println "Done transacting retrieved sources.")

      {:next-state :simplify-conversation-topic
       :updated-context context}))

 #?(:clj
    (defn simplify-conversation-topic-handler [{:keys [params !dh-conn] :as context}]
      (println "Simplifying conversation topic...")

      (simplify-convo-topic !dh-conn params)

      (println "Done simplifying conversation topic.")
 ...
 #?(:clj
    (defn rag-pipeline [params !dh-conn]
      (let [state-machine (-> (create-rag-state-machine)
                              (register-handler :init init-handler)
                              (register-handler :fetch-messages fetch-messages-handler)
                              (register-handler :query-relaxation query-relaxation-handler)
                              (register-handler :lookup-search-phrases lookup-search-phrases-handler)
                              (register-handler :retrieve-chunks retrieve-chunks-handler)
                              (register-handler :format-chunks format-chunks-handler)
                              (register-handler :rerank-chunks rerank-chunks-handler)
                              (register-handler :transact-retrieval-prompt transact-retrieval-prompt-handler)
                              (register-handler :generate-response generate-response-handler)
                              (register-handler :transact-assistant-msg transact-assistant-msg-handler)
                              (register-handler :transact-retrieved-sources transact-retrieved-sources-handler)
                              (register-handler :simplify-conversation-topic simplify-conversation-topic-handler)
        ...
        (if (not-empty (:errors final-state-machine))
          (let [error (-> final-state-machine :errors first :exception)]
            (throw error))
